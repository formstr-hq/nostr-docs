 ...
 * publish a document + metadata and read them back. It does NOT verify
 * signatures (the app sends real signed events, and the client verifies on its
 * end), and it is wiped on restart. Run standalone: `node tests/e2e/relay-server.cjs`.
 */
